...
# IEMOCAP EmoEvaluation label-line, e.g.:
    ...
    with open(eval_path, encoding="utf-8", errors="ignore") as fh:
        for line in fh:
            ...
            if emotion_code not in IEMOCAP_EMOTION_MAP:
                continue  # drop codes outside the 4-class protocol

            dialog = turn_id.rsplit("_", 1)[0]
            gender = turn_id.rsplit("_", 1)[1][0]  # 'F' or 'M' of the speaking turn
            ...
            rows.append(
                {
                    "path": wav_path,
                    "speaker": f"{turn_id[:5]}_{gender}",  # e.g. Ses01_F
                    "emotion_code": emotion_code,
...
def extract_egemaps_features_from_signal(y: np.ndarray, sr: int, smile) -> Tuple[np.ndarray, List[str]]:
    ...
    # Usually one row for Functionals. If more rows appear, average them.
...
def try_load_concept_feature_cache(
    ...
    try:
        ...
        for p in df["path"].astype(str).tolist():
            # Pylance-friendly: select as DataFrame, then convert to 1D vector
            vec = (
                feat_df
                .loc[[p], feature_names]
...
def build_feature_cache(df: pd.DataFrame, cfg: Config) -> Dict[str, Dict[str, np.ndarray]]:
    ...
    for path in tqdm(df["path"].astype(str).tolist(), desc="Extracting logmel/concept features"):
        # Neural input uses peak-normalized audio; concept descriptors use raw fixed audio
        # so intensity/loudness descriptors remain meaningful.
    ...
    # Store feature names in the cache under a reserved key.
    cache["__concept_feature_names__"] = {"names": np.array(final_feature_names, dtype=object)}  # type: ignore
    return cache
